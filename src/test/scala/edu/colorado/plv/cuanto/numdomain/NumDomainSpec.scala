...
class NumDomainSpec extends FlatSpec with Matchers {
  ...
  private def test(man: Manager): Unit = {
    ...
    println("assign-lexp: " + a0.assignCopy(man, 0, linexp, null.asInstanceOf[Abstract0]))
    println("assign-texp: " + a0.assignCopy(man, 0, texp, null.asInstanceOf[Abstract0]))
    println("subst-lexp: " + a0.substituteCopy(man, 0, linexp, null.asInstanceOf[Abstract0]))
    println("subst-texp: " + a0.substituteCopy(man, 0, texp, null.asInstanceOf[Abstract0]))
    assert(a0.isEqual(man, ac))
    ac.assign(man, 0, linexp, null.asInstanceOf[Abstract0])
    assert(ac.isEqual(man, a0.assignCopy(man, 0, linexp, null.asInstanceOf[Abstract0])))
    assert(!ac.isEqual(man, a0))
    ac.assign(man, 0, texp, null.asInstanceOf[Abstract0])
    ac.substitute(man, 0, linexp, null.asInstanceOf[Abstract0])
    ac.substitute(man, 0, texp, null.asInstanceOf[Abstract0])
