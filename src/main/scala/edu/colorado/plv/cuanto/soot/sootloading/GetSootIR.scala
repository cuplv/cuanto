...
object GetSootIR {
  ...
  def getSootIRFromDirectoryOfClassFiles(paths: List[String], main: Option[String]): Scene = {
    try {
      SootLoading.lock.lock()  //Use same lock as sootLoading to avoid concurrent access of global mutible state
      soot.G.reset()
