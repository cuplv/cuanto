...
object SootLoading {
    ...
    def getAnalysisResult[T](paths: List[String], main: Option[String] = None, analysis: Scene => T): Option[T] = {
        ...
        val javaLibraryPath = System.getProperty("java.home") + fileSep + "lib" + fileSep
        val jcePath = javaLibraryPath + "jce.jar"
        val rtPath = javaLibraryPath + "rt.jar"
        
        println(pathSep)
        Scene.v().setSootClassPath("." + pathSep + jcePath + pathSep + rtPath)
